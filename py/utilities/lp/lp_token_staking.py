...
def main():
    while True:
        ...
        if choice == '1':
            balance = get_balance()
            lp_balance = w3.from_wei(balance, 'ether')
            print(f"Your current staked LP token balance: {lp_balance}\n"
                  f"Your LP Value(HONEY) : {round(lp_balance * 100 * 20000, 2)} HONEY")
        
        elif choice == '2':
            token_balance = get_token_balance(MY_WALLET_ADDRESS)
