...
async def call_faucet(address, private_key):
    nonce = await web3.eth.get_transaction_count(address)
    
    txn = await contract.functions.faucet().build_transaction({
        'chainId': 80084, 
        'gas': 200000,
    ...
    print(f"Transaction successful for {address}. Transaction hash: {tx_receipt.transactionHash.hex()}")
...
async def process_account(semaphore, account):
    ...
    async with semaphore:
        ...
        for attempt in range(max_retries):
            try:
                await call_faucet(address, private_key)
                print(f"Faucet request completed for {address}")
                processed_accounts += 1
                break
            except Exception as e:
                print(f"Error requesting faucet for {address} (Attempt {attempt+1}/{max_retries}): {e}")
