...
# 네트워크 연결 설정 (예: BNB Smart Chain Testnet)
rpc_url = 'https://bera-testnet.nodeinfra.com'

# 컨트랙트 주소와 ABI 설정
contract_address = "0xa0525273423537BC76825B4389F3bAeC1968f83F"
contract_abi = [
  {
    "inputs": [
      {
...
async def call_public_mint(address, private_key):
    nonce = await web3.eth.get_transaction_count(address)
    
    # publicMint 함수 호출을 위한 트랜잭션 생성
    txn = await contract.functions.publicMint().build_transaction({
        'chainId': 80084,  # Bera 테스트넷의 체인 ID
        'gas': 200000,
    ...
    print(f"Public mint successful for {address}. Transaction hash: {tx_receipt.transactionHash.hex()}")
...
async def process_account(semaphore, account):
    ...
    async with semaphore:
        ...
        if balance > 0:
            print(f"Skipping account {address} as its balance is greater than 0")
        ...
        for attempt in range(max_retries):
            try:
                await call_public_mint(address, private_key)
                print(f"Public mint completed for {address}")
                processed_accounts += 1
                break
            except Exception as e:
                print(f"Error during public mint for {address} (Attempt {attempt+1}/{max_retries}): {e}")
...
async def main():
    ...
    tasks = [process_account(semaphore, account) for account in accounts]
    await asyncio.gather(*tasks)
    
    await progress_task  # 진행 상황 출력 태스크가 완료될 때까지 기다림

    print("All faucet requests completed.")
