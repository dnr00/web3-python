...
async def execute_and_report(context: ContextTypes.DEFAULT_TYPE):
    ...
    if receipt and receipt.status == 1:
        final_balance = get_balance(TOKEN_ADDRESSES["WBTC"])  # 최종 잔액
        message_text = (f"Contract interaction completed! Transaction Hash: {tx_hash}\n"
                            f"Current Balance: {final_balance * (10**10)} WBTC")
...
async def main():
    ...
    job_queue.run_repeating(callback=execute_and_report, interval=20, first=0)
